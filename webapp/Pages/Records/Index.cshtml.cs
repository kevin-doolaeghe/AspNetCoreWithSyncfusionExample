...
namespace webapp.Pages.Records {
    ...
    public class IndexModel : PageModel {
        ...
        public async Task OnGet() {
            DataSource = await _context.Records.OrderByDescending(x => x.Date).ToListAsync();
        }

        public async Task<JsonResult> OnPostDataSourceAsync([FromBody] DataManagerRequest dm) {
            DataSource = await _context.Records.ToListAsync();
        ...
        public async Task<IActionResult> OnPostCrudUpdateAsync([FromBody] CRUDModel<Record> request) {
            ...
            switch (request.Action) {
                case "insert":
                    _context.Records.Add(request.Value);
                    break;
                case "update":
                    _context.Records.Update(request.Value);
                    break;
                case "remove":
                    long id = long.Parse($"{request.Key}");
                    _context.Records.Remove(_context.Records.Where(x => x.RecordId == id).First());
                    break;
            }
